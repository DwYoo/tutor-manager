...
/** Bullet-key handler for textareas (* → bullet, Enter continues bullets, Tab/Shift+Tab indent/outdent) */
export const bk = (e, val, set, df) => {
  ...
  if (e.key === 'Tab') {
    const ls = val.lastIndexOf('\n', pos - 1) + 1;
    const le = val.indexOf('\n', pos); const lineEnd = le === -1 ? val.length : le;
    const line = val.substring(ls, lineEnd);
    ...
    if (bulletMatch) {
      ...
      if (e.shiftKey) {
        ...
        if (spaces.length > 0) {
          const rm = Math.min(2, spaces.length);
          insertViaExec(ta, line.substring(rm), ls, lineEnd);
          set(ta.value); df?.();
        }
      } else {
        // Tab: indent (add 2 leading spaces)
        insertViaExec(ta, '  ' + line, ls, lineEnd);
        set(ta.value); df?.();
      }
    }
  }
  if (e.key === 'Enter') {
    ...
    const bulletMatch = cl.match(/^( *)(• )/);
    if (bulletMatch) {
      e.preventDefault();
      const indent = bulletMatch[1];
      if (cl.trim() === '•') {
        // Empty bullet: if indented, reduce indent; otherwise remove bullet
        const ls = pos - cl.length;
        if (indent.length >= 2) {
          const newIndent = indent.substring(2);
          insertViaExec(ta, newIndent + '• ', ls, pos);
        } else {
          insertViaExec(ta, '', ls, pos);
        }
        set(ta.value); df?.();
...
export const lessonOnDate = (l, date) => {
  ...
  // recurring_end_date 체크를 시작일(ld===ds) 체크보다 먼저 수행해야
  // 'future' 수정 시 구 시리즈의 시작일이 종료일과 같아지는 경우 올바르게 숨겨짐
  if (l.is_recurring && l.recurring_end_date && ds >= (l.recurring_end_date + "").slice(0, 10)) return false;
