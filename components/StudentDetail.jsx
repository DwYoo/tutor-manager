    ...
    <div className="sd-container" style={{padding:isMobile?16:28}}>
      ...
      <div style={{marginTop:curMain&&curMain.subs.length<=1?20:0}}>
        ...
        {subTab==="calendar"&&(()=>{
          ...
          const gLD=date=>{const ds=fd(date),dw=date.getDay()===0?7:date.getDay();return lessons.filter(l=>{if(l.is_recurring&&l.recurring_exceptions&&l.recurring_exceptions.includes(ds))return false;if(l.date===ds)return true;if(l.is_recurring&&+l.recurring_day===dw){if(ds<l.date)return false;if(l.recurring_end_date&&ds>=(l.recurring_end_date+'').slice(0,10))return false;return true;}return false;});};
        ...
        {subTab==="plan"&&(<div>
          ...
          <div style={{borderTop:"1px solid "+C.bd,paddingTop:20}}>
            ...
            {/* Timeline */}
            {studyPlans.length===0?(<div style={{textAlign:"center",padding:40,color:C.tt,background:C.sf,border:"1px solid "+C.bd,borderRadius:14}}><div style={{fontSize:14}}>아직 지도 방향이 작성되지 않았습니다</div><div style={{fontSize:12,marginTop:4,color:C.tt}}>학생의 지도 방향을 작성해보세요</div></div>):(
              <div style={{position:"relative",paddingLeft:20}}>
                <div style={{position:"absolute",left:5,top:8,bottom:8,width:2,background:C.bl}}/>
