            ...
            <div key={bk} style={{marginBottom:12}}>
              ...
              {exp&&<table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                ...
                <tbody>{items.map(w=>(<tr key={w.id} style={{borderBottom:"1px solid "+C.bl}}>
                  <td style={{padding:"6px 4px"}}>{isParent?<span style={{color:w.chapter?C.ts:"#bbb",fontSize:12,padding:"0 6px"}}>{w.chapter||"단원"}</span>:<input value={w.chapter||""} onChange={e=>updWrong(w.id,"chapter",e.target.value)} style={{border:"none",outline:"none",background:"transparent",color:C.ts,fontSize:12,fontFamily:"inherit",width:"100%",padding:"2px 6px"}} placeholder="단원"/>}</td>
