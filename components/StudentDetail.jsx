...
export default function StudentDetail({ student, onBack, menuBtn }) {
  ...
  const savePlanFields=async()=>{setPlanSaving(true);const{error}=await supabase.from('students').update({plan_strategy:planStrategy,plan_strength:planStrength,plan_weakness:planWeakness}).eq('id',s.id);setPlanSaving(false);if(error)toast?.('계획 저장에 실패했습니다','error');else toast?.('계획이 저장되었습니다');};
  const addPlanComment=async()=>{if(!planComment.trim())return;const{data,error}=await supabase.from('reports').insert({student_id:s.id,title:"",body:planComment,type:"plan",date:fd(new Date()),user_id:user.id}).select().single();if(error){toast?.('기록 추가에 실패했습니다','error');return;}if(data){setPlanComments(p=>[data,...p]);setPlanComment("");}};
  const updatePlanComment=async(id)=>{if(!editCommentText.trim())return;const{error}=await supabase.from('reports').update({body:editCommentText}).eq('id',id);if(error){toast?.('기록 수정에 실패했습니다','error');return;}setPlanComments(p=>p.map(c=>c.id===id?{...c,body:editCommentText}:c));setEditingComment(null);setEditCommentText("");};
