...
export default function Students({onDetail,menuBtn}){
  ...
  const onDR=async(e)=>{e.preventDefault();const fid=dragId,di=dropIdx;setDragId(null);setDropIdx(null);if(!fid||di==null)return;const list=[...activeStudents];const fi=list.findIndex(s=>s.id===fid);if(fi<0)return;const[mv]=list.splice(fi,1);const ai=di>fi?di-1:di;list.splice(ai,0,mv);if(list.every((s,i)=>s.id===activeStudents[i].id))return;const reordered=list.map((s,i)=>({...s,sort_order:i}));setStudents(prev=>[...reordered,...prev.filter(s=>!!s.archived)]);const changed=reordered.filter((s,i)=>s.sort_order!==activeStudents.find(a=>a.id===s.id)?.sort_order);try{await Promise.all(changed.map(s=>supabase.from('students').update({sort_order:s.sort_order}).eq('id',s.id)));}catch{toast?.('순서 저장에 실패했습니다','error');fetchStudents();}};
