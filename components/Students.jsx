...
export default function Students(){
  ...
  const[form,setForm]=useState({name:'',grade:'',subject:'',school:'',address:'',phone:'',parent_phone:'',fee:'',fee_per_class:'',birth_date:''});
  ...
  const fetchStudents=async()=>{setFetchError(false);try{const[sRes,lRes]=await Promise.all([supabase.from('students').select('*').order('created_at'),supabase.from('lessons').select('*').order('date')]);if(sRes.error||lRes.error){toast?.('데이터를 불러오지 못했습니다','error');setFetchError(true);}const stuData=sRes.data||[];const redistributed=await redistributeColors(stuData);setStudents(redistributed);setLessons(lRes.data||[]);}catch{toast?.('데이터를 불러오지 못했습니다','error');setFetchError(true);}setLoading(false);};
  const redistributeColors=async(stuData)=>{const sorted=[...stuData].sort((a,b)=>{const sa=a.sort_order??Infinity,sb=b.sort_order??Infinity;return sa!==sb?sa-sb:new Date(a.created_at)-new Date(b.created_at);});const indexMap=new Map(sorted.map((s,i)=>[s.id,i%SC.length]));const toUpdate=stuData.filter(s=>s.color_index!==indexMap.get(s.id));if(!toUpdate.length)return stuData;await Promise.all(toUpdate.map(s=>supabase.from('students').update({color_index:indexMap.get(s.id)}).eq('id',s.id)));return stuData.map(s=>({...s,color_index:indexMap.get(s.id)??s.color_index}));};
  ...
  const openAdd=()=>{setEditStu(null);setForm({name:'',grade:'',subject:'',school:'',address:'',phone:'',parent_phone:'',fee:'',fee_per_class:'',birth_date:''});setShowAdd(true);};
  const openEdit=(s,e)=>{e.stopPropagation();setEditStu(s);setForm({name:s.name||'',grade:s.grade||'',subject:s.subject||'',school:s.school||'',address:s.address||'',phone:s.phone||'',parent_phone:s.parent_phone||'',fee:String(s.fee||''),fee_per_class:String(s.fee_per_class||''),birth_date:s.birth_date||''});setShowAdd(true);};
          ...
          <div onClick={e=>e.stopPropagation()} className="detail-modal" style={{background:C.sf,borderRadius:16,width:"100%",maxWidth:440,padding:28,boxShadow:"0 20px 60px rgba(0,0,0,.15)",maxHeight:"90vh",overflow:"auto"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              {[
                ...
                {k:'address',l:'주소',ph:'예: 서울시 강남구 역삼동'},
