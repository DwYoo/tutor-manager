...
export default function Students({onDetail,menuBtn}){
  ...
  const fetchStudents=useCallback(async()=>{
    ...
    const isArchived=s=>Boolean(s.is_archived)||Boolean(s.archived_at);
    setStudents(allStudents.filter(s=>!isArchived(s)));
    setArchivedStudents(allStudents.filter(isArchived));
  ...
  const saveStudent=async()=>{
    ...
    if(editStu){
      ...
      const{error}=await supabase.from('students').insert({...full,color_index:(students.length+archivedStudents.length)%8,fee_status:'unpaid',user_id:user.id});
      if(!error)fetchStudents();
    }
  ...
  const setArchiveStatus=async(id,archived)=>{
    const primary=archived?{is_archived:true}:{is_archived:false};
    const fallback=archived?{archived_at:new Date().toISOString()}:{archived_at:null};
    let lastErr=null;
    for(const payload of [primary,fallback]){
  ...
  const archiveStudent=async(id,e)=>{e.stopPropagation();if(!confirm('학생을 보관하시겠습니까?\nStudents 목록에서 숨겨지고 나중에 복원할 수 있습니다.'))return;const err=await setArchiveStatus(id,true);if(err){alert(`보관에 실패했습니다: ${err.message}`);return;}fetchStudents();};
  const restoreStudent=async(id)=>{const err=await setArchiveStatus(id,false);if(err){alert(`복원에 실패했습니다: ${err.message}`);return;}fetchStudents();};
